...
class Setter(ABC):
    def __init__(self, name: str):
        self.name = name
        self.reward = 'sparse' # in {'sparse', 'dense', 'proba'}
    ...
    def compute_reward(self, obs, desired_goals, **kwargs):
        if self.reward == 'sparse':
            d = np.linalg.norm(obs['achieved_goal'] - desired_goals, axis=-1)
            ...
            reward = -np.linalg.norm(obs['achieved_goal'] - desired_goals, axis=-1
                                    ).astype(np.float32
                                    ).reshape((self.num_envs, 1))
                                    
        elif self.reward == 'proba':
            
            goal_dim = desired_goals.shape[-1]
            
            reward = np.zeros((self.num_envs, 1))
            
            for i in range(self.num_envs):
                mu_ag = torch.from_numpy(obs['mu_ag'][i])
                mu_dg = torch.from_numpy(obs['mu_dg'][i])
                sig_ag = torch.from_numpy(obs['sig_ag'][i])
                sig_dg = torch.from_numpy(obs['sig_dg'][i])
                
                d_ag = MultivariateNormal(mu_ag, sig_ag**2*torch.eye(goal_dim))
                d_dg = MultivariateNormal(mu_dg, sig_dg**2*torch.eye(goal_dim))
                
                kl_div = kl.kl_divergence(d_ag,d_dg)
               
                reward[i] = -kl_div.numpy()
...
class SvggSetter(Setter, ABC):
    ...
    def step(
        ...
        if not eval_mode:
            ...
            # Optimize prior
            if not self.steps % max(self.prior_oe, 1) and self.prior_need_opti:
