...
from xpag.tools.models import train_vae_model, plot_ae_outputs, ae_plot_gen, compare_vae_obs, Prioritized_Replay
...
def learn(
    ...
    *,
    ...
    if vae_obs is not None:
        ...
        vae_opt = torch.optim.Adam(vae_obs.parameters(), lr=2e-3, weight_decay=1e-5)
        vae_plot_step = 0
        vae_ready = False
        ...
        if vae_pretrain_oracle and not vae_ready:
                ...
                plot_latent_codes(env=eval_env, 
                                  nb_goals=50, 
                                  file_path=DIST_EVO_PATH+'/latent_codes', 
                                  goals_fn=force_eval_goal_fn
                                )
    ...
    for i in range(max_steps // env_info["num_envs"]):
        ...
        if not i % max(evaluate_every_x_steps // env_info["num_envs"], 1):
            ...
            if mult_eval_env is not None:
                ...
                # Plot grid success coverage
                success = info["is_success"].astype('float32')
                plot_grid_coverage(eval_env, 
                                   success, 
                                   i* env_info["num_envs"], 
                                   grid_succ_plot_step, 
                                   save_dir, 
                ...
                if vae_obs is not None:
                    # Plot latent grid success coverage
                    success_latent = info["is_success_latent"].astype('float32')
                    plot_grid_coverage(eval_env, 
                                       success_latent, 
                                       i*env_info["num_envs"], 
                                       grid_succ_plot_step, 
                                       save_dir, 
                                       tag='grid_eval_latent',
                                       writer=None
                                    )
        ...
        if vae_obs is not None:
            ...
            if not i % max(vae_oe // env_info["num_envs"], 1) and i > 0 and not vae_pretrain_oracle:
                ...
                # Prioritized Variance Sampling
                #with torch.no_grad():
                #    _ = eval_env.embed(obs_batch, train=False)
                #var = eval_env.embed.sigma
                #probas = F.softmax(10*var.mean(1))
                #distrib = Categorical(probas)
                #idxs = distrib.sample((batch_size,)).numpy()
                #obs_batch = obs_batch[idxs]
                ...
                if not i % max(20000 // env_info["num_envs"], 1):
                    ...
                    plot_latent_codes(env=eval_env, 
                                  nb_goals=50, 
                                  file_path=LATENT_VARIANCE+f'/step_{i*env_info["num_envs"]}', 
                                  goals_fn=force_eval_goal_fn
                                )
        ...
        if i * env_info["num_envs"] < start_training_after_x_steps:
            ...
            if i > 0:
                for _ in range(int(max(gd_steps_per_step * env_info["num_envs"], 1))):
                    ...
                    _ = agent.train_on_batch(samples)
