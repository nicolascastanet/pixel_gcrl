    ...
    while not np.all(done):
        ...
        if goal_fn is not None:
            if env_info["from_pixels"]:
                ...
                dist_latent = np.linalg.norm(next_observation["achieved_goal"] - desired_goals, axis=-1)
                succ_latent = (dist_latent < 0.15).reshape(-1,1)
                info["is_success_latent"] = np.copy(succ_latent)
            else:
        ...
        observation = next_observation
        list_obs.append(observation)
    
    
    update_csv("average success", info["is_success"].mean(), steps, save_dir, writer)
    if "is_success_latent" in info:
        update_csv("average success_latent", info["is_success_latent"].mean(), steps, save_dir, writer)
    return info#, list_obs