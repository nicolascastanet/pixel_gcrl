...
def train_vae_model(
    ...
    for epoch in range(nb_steps):
        ...
        for x in dataloader:
            ...
            x_input = x[0].to(device)
            
            #import ipdb;ipdb.set_trace()
            x_hat = vae(x_input)
            
            #import ipdb;ipdb.set_trace()
            #loss = F.mse_loss(x_hat, x_input, size_average=False) + vae.encoder.kl
            
            # Evaluate loss
            loss = F.binary_cross_entropy(x_hat,x_input, size_average=False) + beta*vae.encoder.kl
            # Backward pass
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def plot_ae_outputs(epoch, encoder, decoder, path, dataset, t_idx, n=10):
    ...
    for i in range(n):
      ...
      decoder.eval()
      with torch.no_grad():
         rec_img  = decoder(encoder(img,train=False))
...
def ae_plot_gen(step, plot_step, vae, path, writer=None, input_obs = None, env=None):
    device = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")
    vae.eval()
    d = vae.latent_dim    
    ...
    with torch.no_grad():
        ...
        if input_obs is not None:
            pixel_obs = np.swapaxes(env.convert_2D_to_pixel(input_obs),3,1)
            pixel_obs = torch.from_numpy(pixel_obs).to(device).type(torch.float)
            _ = vae.encoder(pixel_obs, train=False)
            
            sigma = vae.encoder.sigma
            mu = vae.encoder.mu
            latent = mu + sigma*vae.encoder.N.sample((128,d))
...
def compare_vae_obs(step,
    ...
    num_obs = pixel_obs.shape[0]

    if latent_obs is not None:
        latent_gen = latent_obs
    else:
        latent_gen = torch.randn(500, d)#, device=device)
    ...
    with torch.no_grad():
        ...
        latent_encode = encoder(pixel_obs,train=False)
    ...
    for (lg,pixel_g) in zip(latent_gen,img_gen):
        # Compute L2 norm between latent codes
        diff = np.linalg.norm((latent_encode - lg).numpy(), axis=-1)
        diff_2 = np.linalg.norm((pixel_obs.reshape(num_obs, -1) - pixel_g.flatten()).numpy(), axis=-1)
        #import ipdb;ipdb.set_trace()
        argmin = np.argmin(diff_2)
...
class Prioritized_Replay():
    def __init__(self,buffer, rank_method='dense'):
    ...
    def sample_uniform(self, batch_size):
        ...
        X_train = transitions["next_observation.achieved_goal"]
        ...
        entropy_rank = rankdata(pred, method=self.rank_method)
        entropy_rank = (entropy_rank - 1)
        
        p_states = entropy_rank / entropy_rank.sum()
        num_states = p_states.shape[0]
        
        batch_tidx = np.random.choice(num_states, size=batch_size, p=p_states)
        
        return transitions["next_observation.init_obs"][batch_tidx]
