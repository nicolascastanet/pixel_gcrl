...
def custom_vec_env(
    ...
    if cnn_policy:
        env = PixelWrapper(env, dummy_env)
        eval_env = PixelWrapper(dummy_env, dummy_env)
        eval_env_mult = PixelWrapper(eval_env_mult, dummy_env)
        
    elif embed is not None:
        env = LatentRewardWrapper(EmbedVecWrapper(env, dummy_env, embed), reward=reward)
        eval_env = LatentRewardWrapper(EmbedVecWrapper(dummy_env, dummy_env, embed), reward=reward)
        eval_env_mult = LatentRewardWrapper(EmbedVecWrapper(eval_env_mult, dummy_env, embed), reward=reward)
...
class LatentRewardWrapper(gym.Wrapper):
    """_summary_
    Compute the reward in the latent space after pixel observation transformation 
    Args:
        gym (_type_): _description_
    """
    def __init__(self, env: VectorEnv, reward : str):
        super().__init__(env)
        self.latent_dist_threshold = 0.15
        self.reward = reward
        
        
    def step(self, action):
        obs, _, terminated, truncated, info_ = self.env.step(action)
        
        if "sig_ag" in obs:
            input_dict = {"sig_ag" : obs["sig_ag"],
                            "mu_ag" : obs["mu_ag"]
                        }
        else:
            input_dict = None
        ...
        if self.reward in {'proba', 'dense'}:
            info_["is_success"] = reward + 1
    ...
    def compute_reward(self, achieved_goals, desired_goals, info, *args, **kwargs):
        
        if self.reward == 'sparse':
            d = np.linalg.norm(achieved_goals - desired_goals, axis=-1)
            reward = -(d >= self.latent_dist_threshold).astype(np.float32
                        )
        ...
        elif self.reward == 'proba':
                    
            sig_ag = kwargs["other_args"]["sig_ag"]
            mu_ag = kwargs["other_args"]["mu_ag"]
            
            
            d = np.all(abs(desired_goals - mu_ag) < sig_ag*5,axis=-1)
            reward = np.array(d-1).astype(np.float32)
            
            #import ipdb;ipdb.set_trace()
            #if len(mu_ag.shape) > 1:
            #    import ipdb;ipdb.set_trace()
...
class EmbedVecWrapper(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        with torch.no_grad():
            batch_latent_obs = self.embed(batch_pixels_obs,train=False)
        ...
        mu_obs, mu_dg, mu_ag = torch.split(self.embed.mu, self.num_envs)
        sig_obs, sig_dg, sig_ag = torch.split(self.embed.sigma, self.num_envs)
        
        if self.num_envs == 1:
            observation = observation.squeeze()
            desired_goal = desired_goal.squeeze()
            achieved_goal = achieved_goal.squeeze()
            mu_obs, mu_dg, mu_ag = mu_obs.squeeze(), mu_dg.squeeze(), mu_ag.squeeze()
            sig_obs, sig_dg, sig_ag = sig_obs.squeeze(), sig_dg.squeeze(), sig_ag.squeeze()
            ...
            assert mu_obs.shape == torch.Size([self.num_envs, self.latent_dim])
            assert sig_obs.shape == torch.Size([self.num_envs, self.latent_dim])
        ...
        # Record latent space gausian parameters
        obs['mu_obs'], obs['mu_dg'], obs['mu_ag'] = mu_obs.numpy(), mu_dg.numpy(), mu_ag.numpy()
        obs['sig_obs'], obs['sig_dg'], obs['sig_ag'] = sig_obs.numpy(), sig_dg.numpy(), sig_ag.numpy()
        
        return obs
    
    
    def convert_2D_to_pixel_bis(self, obs, rg=5):
        ...
        height, width, _ = fig_array_pixels.shape[1:]
        # Create arrays representing the indices of the rows and columns for each image
        rows, cols = np.ogrid[:height, :width]
        # Extract center coordinates for all images
        center_rows, center_cols = coord[:, 0], coord[:, 1]
        # Calculate the distance from each pixel to the center coordinates for all images
        distances = np.sqrt((rows - center_rows[:, np.newaxis, np.newaxis])**2 +
                            (cols - center_cols[:, np.newaxis, np.newaxis])**2)
        # Create a mask for pixels within the specified radius for all images
        mask = distances <= rg
        # Set the patch to red in the modified images
        fig_array_pixels[mask] = [256,0,0]
        
        return fig_array_pixels/256

        
    def convert_2D_to_pixel(self, obs, rg=5):
...
class PixelWrapper(gym.ObservationWrapper):
    ...
    Args:
        gym (_type_): _description_
    """
    def __init__(self, env: VectorEnv, dummy_env):
        super().__init__(env)
        self.env = env
        self.rgb_maze = dummy_env.get_rgb_maze()
    ...
    def observation(self,obs):
        ...
        batch_obs = np.concatenate((observation, desired_goal, achieved_goal)).reshape(-1,2)
        batch_pixels_obs = np.swapaxes(self.convert_2D_to_pixel(batch_obs),3,1)
        obs_ag_dg = np.split(batch_pixels_obs, 3)
        
        obs['observation'] = obs_ag_dg[0]
        obs['desired_goal'] = obs_ag_dg[1]
        obs['achieved_goal'] = obs_ag_dg[2]
